...
async function cargarListaCategoriasAdmin(filtro = '') {
  ...
  try {
    ...
    const { data: todosSubs } = await supabaseClient
      .from('subproductos_pos')
      .select('categoria_id');

    const conteoPorCat = {};
    (todosSubs || []).forEach(s => {
      conteoPorCat[s.categoria_id] = (conteoPorCat[s.categoria_id] || 0) + 1;
    });

    contenedor.innerHTML = lista.map(c => {
      const totalProds = conteoPorCat[c.id] || 0;
...
function nuevaCategoria() {
  ...
  const secSubs = document.getElementById('adminSubproductosCategoria');
...
async function editarCategoria(id) {
  ...
  await cargarSubproductosDeCategoria(cat.id);
...
async function eliminarCategoria(id) {
  ...
  const totalProds = (subproductosCategoriaActual || []).length;
...
// =========================================================
// CARGAR SUBPRODUCTOS DE UNA CATEGORÍA
// =========================================================
async function cargarSubproductosDeCategoria(categoriaId) {
  const contenedor = document.getElementById('adminListaSubproductosCat');
  if (!contenedor) return;

  contenedor.innerHTML = '<div style="text-align:center; padding:10px; color:#888;">Cargando...</div>';
  ...
  try {
    ...
    subproductosCategoriaActual = data || [];

    if (subproductosCategoriaActual.length === 0) {
      contenedor.innerHTML = '<div style="text-align:center; padding:16px; color:#888; font-size:13px;">Sin subproductos. Agrega uno abajo. 👇</div>';
    } else {
      contenedor.innerHTML = subproductosCategoriaActual.map(s => `
        <div class="admin-subproducto-item" data-id="${s.id}">
          <input type="text" class="admin-sub-input" value="${s.nombre}" 
                 onchange="actualizarSubproducto(${s.id}, 'nombre', this.value)"
                 placeholder="Nombre">
          <input type="number" class="admin-sub-input-precio" value="${parseFloat(s.precio_usd) || 0}" step="0.01" min="0"
                 onchange="actualizarSubproducto(${s.id}, 'precio_usd', this.value)"
                 placeholder="Precio">
          <input type="number" class="admin-sub-input-orden" value="${s.orden || 0}" step="1"
                 onchange="actualizarSubproducto(${s.id}, 'orden', this.value)"
                 placeholder="Orden">
          <button class="admin-sub-btn-eliminar" onclick="eliminarSubproductoDeCategoria(${s.id})" title="Eliminar">🗑️</button>
        </div>
      `).join('');
    }
    ...
    contenedor.innerHTML = '<div style="text-align:center; padding:10px; color:#f87171;">Error al cargar</div>';
  }
}
...
// AGREGAR SUBPRODUCTO
// =========================================================
async function agregarSubproductoDesdeCategoria() {
  if (!categoriaEditando) {
    alert('Guarda primero la categoría');
    return;
  }

  const nombreInput = document.getElementById('adminNuevoSubNombre');
  const precioInput = document.getElementById('adminNuevoSubPrecio');
  
  const nombre = nombreInput.value.trim();
  const precio = parseFloat(precioInput.value) || 0;

  if (!nombre) {
    alert('El nombre del subproducto es obligatorio');
    return;
  }
  ...
  try {
    const { error } = await supabaseClient
      ...
      .insert([{
        nombre: nombre,
        categoria_id: categoriaEditando.id,
        precio_usd: precio,
        orden: subproductosCategoriaActual.length + 1,
        activo: true
      }]);
    ...
    await cargarSubproductosDeCategoria(categoriaEditando.id);
    await cargarListaCategoriasAdmin();
    await cargarProductos();

  } catch (error) {
    console.error("❌ Error agregando subproducto:", error);
    alert('Error: ' + error.message);
  }
}
...
// =========================================================
async function actualizarSubproducto(id, campo, valor) {
  try {
    const datos = {};
    ...
    } else if (campo === 'precio_usd') {
      datos.precio_usd = parseFloat(valor) || 0;
    } else if (campo === 'orden') {
    ...
    const sub = subproductosCategoriaActual.find(s => s.id === id);
    if (sub) Object.assign(sub, datos);
...
// =========================================================
async function eliminarSubproductoDeCategoria(id) {
  const sub = subproductosCategoriaActual.find(s => s.id === id);
  if (!sub) return;
  ...
  try {
    ...
    showToast(`✅ "${sub.nombre}" eliminado`, 'success');
